<template>
    <div>
        ...
        <table>
            ...
            <tbody>
                <tr v-for="(submission, index) in kidsSubmissions" :key="index" :class="{ 'read-row': submission.isRead }">
        ...
        <button @click="openAddModal" class="add-button">{{ $t('actions.addNewKidsSubmission') }}</button>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        async toggleReadStatus(submissionId) {
            await this.toggleSubmissionReadStatus({ submissionId, type: 'kids' });
        }
    }
};
</script>
