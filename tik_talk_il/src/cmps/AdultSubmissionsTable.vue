<template>
    <div>
        ...
        <table>
            ...
            <tbody>
                <tr v-for="(submission, index) in adultSubmissions" :key="index" :class="{ 'read-row': submission.isRead }">
        ...
        <button @click="openAddModal" class="add-button">{{ $t('actions.addNewAdultSubmission') }}</button>
    </div>
</template>
...
export default {
    computed: {
        ...mapGetters('submissions', ['adultSubmissions']),
    },
    methods: {
        ...mapActions('submissions', ['saveSubmission', 'deleteSubmission', 'toggleSubmissionReadStatus']),
        openEditModal(submission) {
            this.$emit('edit', submission, 'adult');
        },
        openAddModal() {
            this.$emit('add', 'adult');
        },
        async handleDeleteSubmission(submissionId) {
            await this.deleteSubmission({ submissionId, type: 'adult' });
        },
        async toggleReadStatus(submissionId) {
            await this.toggleSubmissionReadStatus({ submissionId, type: 'adult' });
        }
    }
};
</script>
